...
#[tauri::command]
pub(crate) fn search_notes(state: State<'_, DbState>, query: String, limit: Option<i64>) -> Result<NotesPage, String> {
    let conn = state.conn.lock().map_err(|error| error.to_string())?;
...
#[tauri::command]
pub(crate) fn find_note_by_title(state: State<'_, DbState>, title: String) -> Result<Option<NoteDto>, String> {
    let conn = state.conn.lock().map_err(|error| error.to_string())?;
...
#[tauri::command]
pub(crate) fn update_note(state: State<'_, DbState>, input: UpdateNoteInput) -> Result<NoteDto, String> {
    let conn = state.conn.lock().map_err(|error| error.to_string())?;
